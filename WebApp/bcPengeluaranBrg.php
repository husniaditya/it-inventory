    ...
    <div class="panel-default">
        ...
        if (isset($_POST["btnPreview"]) || isset($_POST["btnCSV"] ) || isset($_POST["btnPDF"] )) {
           $tglAwal = new DateTime($_POST["tglAwal"]);
            ...
            $query = "SELECT @num := @num + 1 AS no, temp.* from (select jenis, nodoc, datedoc, sjh, sjhdate, supplier, inv, remark, qty, qty1, unit, val, sub, form, loc FROM bckeluar ";
               $query .= "   WHERE sjhdate >=" . $tglAwal . " and sjhdate < adddate(" . $tglAkhir . ",1)";

            if ($_POST["txtSupA"] == "") {
                if ($_POST["txtSupZ"] == "")
                    $query .= "AND (sub BETWEEN '' and 'zzz') ";
                else
                    $query .= "AND (sub BETWEEN '" . $_POST["txtSupZ"] . "' and '" . $_POST["txtSupZ"] . "') ";
            }
            else {
                if ($_POST["txtSupZ"] == "")
                    $query .= "AND (sub BETWEEN '" . $_POST["txtSupA"] . "' and '" . $_POST["txtSupA"] . "') ";
                else
                    $query .= "AND (sub BETWEEN '" . $_POST["txtSupA"] . "' and '" . $_POST["txtSupZ"] . "') ";
            }

            if ($_POST["txtInvA"] == "") {
                if ($_POST["txtInvZ"] == "")
                    $query .= "AND (inv BETWEEN '' and 'zzz') ";
                else
                    $query .= "AND (inv BETWEEN '" . $_POST["txtInvA"] . "' and '" . $_POST["txtInvZ"] . "') ";
            }
            else {
                if ($_POST["txtInvZ"] == "")
                    $query .= "AND (inv BETWEEN '" . $_POST["txtInvA"] . "' and '" . $_POST["txtInvA"] . "') ";
                else
                    $query .= "AND (inv BETWEEN '" . $_POST["txtInvA"] . "' and '" . $_POST["txtInvZ"] . "') ";
            }
            ...
            if ($_POST["txtDocA"] == "") {
                if ($_POST["txtDocZ"] == "")
                    $query .= "AND (ifnull(nodoc,'') BETWEEN '' and 'zzz') ";
                else
                    $query .= "AND (nodoc BETWEEN '" . $_POST["txtDocA"] . "' and '" . $_POST["txtDocZ"] . "') ";
            }
            else {
                if ($_POST["txtDocZ"] == "")
                    $query .= "AND (nodoc BETWEEN '" . $_POST["txtDocA"] . "' and '" . $_POST["txtDocA"] . "') ";
                else
                    $query .= "AND (nodoc BETWEEN '" . $_POST["txtDocA"] . "' and '" . $_POST["txtDocZ"] . "') ";
            }
            ...
            if ($_POST["jnsA"] == "") {
                if ($_POST["jnsZ"] == "")
                    $query .= "AND (ifnull(jenis,'') BETWEEN '' and 'zzz')";
                else
                    $query .= "AND (jenis BETWEEN '" . $_POST["jnsA"] . "' and '" . $_POST["jnsZ"] . "') ";
            }
            else {
                if ($_POST["jnsZ"] == "")
                    $query .= "AND (jenis BETWEEN '" . $_POST["jnsA"] . "' and '" . $_POST["jnsA"] . "') ";
                else
                    $query .= "AND (jenis BETWEEN '" . $_POST["jnsA"] . "' and '" . $_POST["jnsZ"] . "') ";
        ...
        if (isset($_POST["btnPreview"]) || isset($_SESSION["searchQuery"]) ) {
           ...
           $column = new GridDateTimeColumn();
            ...
            $column = new GridBoundColumn();
            $column->HeaderText = "JENIS FORM";
            $column->DataField = "form";
            $column->AllowExporting = true;
            $column->Valign = "top";
            $grid->MasterTable->AddColumn($column);
        ...
        if (isset($_POST["btnCSV"]) ) {
            ...
            if ($num > 0) {
                ...
                $html .= "<tr>"
                   . "<th>NO</th><th>JENIS DOKUMEN</th><th>NO DOKUMEN PABEAN</th>"
                   . "<th>TGL DOK. PABEAN</th><th>NO BUKTI PENGELUARAN</th><th>TGL BUKTI PENGELUARAN</th>"
                   . "<th>JENIS FORM></th><th>PEMBELI/PENERIMA</th><th>KODE BARANG</th><th>NAMA BARANG</th>"
                   . "<th>SATUAN</th><th>QTY</th><th>NILAI BARANG</th>"
                . "</tr>";
                for ($i = 0; $i < $num; $i++) {
        ...
        if(isset($_POST["btnPDF"])){
            ...
            $tablehead = array('NO', 'JENIS  DOK', 'NO DOK PABEAN', 'TGL DOK PABEAN',
                'NO BUKTI PENGELUARAN', 'TGL BUKTI PENGELUARAN', 'FORM', 'PEMASOK',
                'KODE BARANG', 'NAMA BARANG', 'SAT', 'QTY', 'NILAI BARANG');
