...
class HugeMySQLDataSource extends DataSource
{
	...
	function _select_sql_break($_statement)
	{
		...
		$_target = array("select"=>"","from"=>"","where"=>"","order by"=>"","group by"=>"");
	...
	function GetData($_start=0,$_count=9999999)
	{
		...
		if ($_orderby!="")
		{
			if($_command_parts["orderby"]=="")
			{
				$_command_parts["orderby"] = "ORDER BY ".substr($_orderby,2);			
			}
			else
			{
				$_command_parts["orderby"].=$_orderby;
			}		
		}			
	...
	function Insert($_associate_array)
	{
		...
		foreach($_associate_array as $_key=>$_value)
		{
			...
			{
				$_insert_commands[$i] = _replace("@".$_key,_slash_encode($_value),$_insert_commands[$i]);
			}
	...
	function Update($_associate_array)
	{
		...
		foreach($_associate_array as $_key=>$_value)
		{
			...
			{
				$_update_commands[$i] = _replace("@".$_key,_slash_encode($_value),$_update_commands[$i]);
			}
		}
