                ...
                <div class="modal-body">
                    <table id="lookupIA" class="table table-bordered table-hover table-striped">
                        <thead>
                         ...
                         <tbody>
                            <?php
                            $sql = mysql_query('SELECT inv AS Kode, name AS Persediaan FROM inv WHERE flag = 0 and left(inv,2) = 51 ;');
...
<div id="page-wrapper">   
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header" style="font-size: 25px;">BC Mutasi Barang Jadi</h1>
        </div>
    </div>
    ...
    <div class="panel-default">        
        ...
        if (isset($_POST["btnPreview"]) || isset($_POST["btnCSV"] ) || isset($_POST["btnPDF"] )) {
            ...
            $query .= "   format(if(sum(ifnull(format(qopn,2),0)) <> 0 , sum(s_awal) + sum(qdebet) - sum(qkredit) +  sum(qadjust) + sum(ifnull(qopn,0)),0),2) as opname, '-' as ket from ";
            $query .= "( ";
            ...
            $query .= "      where rin.period='" . $period . "' and left(rin.inv,2)='51' ";
            $query .= "     union all ";
            $query .= "     select bckeluar.inv,bckeluar.remark, bckeluar.qty as qlast , bckeluar.val as vlast, ";
            $query .= "     '',inv.unit2,inv.unit ";
            $query .= "       from bckeluar left outer join inv on bckeluar.inv=inv.inv ";
            $query .= "      where sjhdate < " . $tglAwal . " and month(sjhdate)=month(" . $tglAwal . ") and year(sjhdate)=year(" . $tglAwal . ") and left(bckeluar.inv,2)='51' ";
            $query .= "  ) as oawal ";
            if ($_POST["txtInvA"] == "") {
                if ($_POST["txtInvZ"] == "")
            ...
            $query .= " group by inv ";
            $query .= "  union all ";
            $query .= " select inv.name as invname, ind.inv, convert('' using latin1) as loc, ";
            $query .= "       ind.date as date, ind.jurnal as jurnal, ifnull(ind.rem,'') as rem, 0 as s_awal, ";
            $query .= "       if (ind.dk='D' and ind.subjurnal not in ('OPN','JIN','BPP','RKA') , ind.qty , 0) as qdebet, ";
            $query .= "       if (ind.dk='K' and ind.subjurnal not in ('OPN','JIN','BPP','RKA') , ind.qty , 0) as qkredit, ";
            $query .= "       if (ind.subjurnal in ('JIN','BPP','RKA') ,if(ind.dk='K', -ind.qty , ind.qty),0) as qadjust, ";
            ...
            $query .= "       where left(ind.inv,2) = '51' and (ind.date>=" . $tglAwal . " and ind.date<adddate(" . $tglAkhir . ",1)) ";
            if ($_POST["txtInvA"] == "") {
                if ($_POST["txtInvZ"] == "")
            ...
            $query .= " and  left(ind.inv,1) = '5' and left(ind.jurnal,3) in ('LHP','OPN','JIN','BPP','SJL','LPR','LPB') ";
            $query .= " union all " ;
            $query .= " select bckeluar.remark as invname,bckeluar.inv,'' as loc, " ;
            $query .= " bckeluar.sjhdate,bckeluar.sjh,'' as rem,0 as s_awal,  " ;
            $query .= " if (inv='911010001',bckeluar.qty,0) as qdebet,bckeluar.qty as qkredit,0 as qadjust,0 as opn,  " ;
            $query .= " 'K' as dk,bckeluar.unit,1 as urut,'' as nodsg  " ;
            $query .= " from bckeluar  " ;
            $query .= " where bckeluar.nodoc between '' and 'z'  " ;
             if ($_POST["txtInvA"] == "") {
                if ($_POST["txtInvZ"] == "")
            ...
            $query .= "  and sjhdate >=" . $tglAwal . " and sjhdate < adddate(" . $tglAkhir . ",1) ";
            $query .= "  and  (left(bckeluar.inv,2) = '41' or inv ='911010001') " ;
        ...
        if (isset($_POST["btnPreview"]) || isset($_SESSION["searchQuery"]) ) {
            ...
            echo '<p><strong>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</strong></p>';
            echo '<p>KAWASAN BERIKAT Wage Biro Technic</p>';
            echo '<p>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</p>';
        ...
        if (isset($_POST["btnCSV"])) {
            ...
            if ($num > 0) {
                ...
                $html .= '<p><strong>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</strong></p>';
                $html .= '<p>KAWASAN BERIKAT Wage Biro Technic</p>';
                $html .= '<p>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</p>';
                ...
                header("Content-Disposition: attachment; filename=BC_Laporan_PertanggungJawaban_Mutasi_Barang_Jadi.xls");
        ...
        if (isset($_POST["btnPDF"])) {
            ...
            $header = '<br /><br /><h2><strong>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</strong></h2>';
            $header .= '<h3>KAWASAN BERIKAT Wage Biro Technic</h3>';
            $header .= '<h3>LAPORAN PERTANGGUNG JAWABAN MUTASI BARANG JADI</h3>';
